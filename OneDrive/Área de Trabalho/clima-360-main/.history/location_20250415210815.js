  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(
      function(position) {
        ...
        fetch("http://localhost:3000/clima-por-coordenada", {
          method: "POST",
          headers: {
        ...
        .then(data => {
          ...
          // Aqui você pode exibir os dados na interface do app
        })
        .catch(err => console.error("Erro ao buscar clima:", err));
      },
      function (error) {
        console.error("Erro ao acessar localização:", error.message);
      }
    );
  } else {
